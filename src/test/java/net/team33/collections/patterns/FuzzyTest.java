package net.team33.collections.patterns;

import org.junit.Test;

import java.util.Date;
import java.util.Objects;
import java.util.Optional;
import java.util.stream.Stream;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.fail;

public class FuzzyTest {

    public static final Object[] VALUES = {"Ein String", 3.141592654, 278, false, null, new Date()};

    @Test
    public final void testToString() {
        Stream.of(VALUES)
                .forEach(value -> assertEquals(
                        "<" + Objects.toString(value) + ">",
                        Fuzzy.of(value).toString()));
    }

    @Test
    public final void testHashCode() {
        Stream.of(VALUES)
                .forEach(value -> assertEquals(
                        Objects.hashCode(value),
                        Fuzzy.of(value).hashCode()));
    }

    @Test
    public final void testEquals() {
        Stream.of(VALUES).forEach(left ->
                Stream.of(VALUES).forEach(right ->
                        assertEquals(
                                Objects.equals(left, right),
                                Fuzzy.of(left).equals(Fuzzy.of(right)))));
    }

    @Test
    public final void testWhen() {
        Fuzzy.of(3.141592654)
                .when(Integer.class).then(value -> fail("should be skipped <Integer:" + value + ">"))
                .when(Double.class).then(value -> assertEquals(Double.class, value.getClass()))
                .when(String.class).then(value -> fail("should be skipped <String:" + value + ">"));
    }

    @Test
    public final void testWhenThen() {
        assertEquals("Double <3.141592654>", Fuzzy.of(3.141592654)
                .when(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElse("Anything else"));
    }

    @Test
    public final void testOrWhenDouble() {
        assertEquals("Double <3.141592654>", Fuzzy.of(3.141592654)
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orThrow(MyException::new));
    }

    @Test
    public final void testWhenNull() {
        assertEquals("<null>", Fuzzy.of(null)
                .when(null).reply(core -> "<null>")
                .orWhen(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElseGet(() -> "Anything else"));
    }

    @Test
    public final void testOrWhenNull() {
        assertEquals("<null>", Fuzzy.of(null)
                .when(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(null).reply(core -> "<null>")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElse("Anything else"));
    }

    @Test
    public final void testWhenNullOptional() {
        assertEquals(Optional.empty(), Fuzzy.of(null)
                .when(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .optional());
    }

    @Test
    public final void testWhenNullOrElseGet() {
        assertEquals("Anything else", Fuzzy.of(null)
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElseGet(() -> "Anything else"));
    }

    @Test
    public final void testOrWhenString() {
        assertEquals("String <3.141592654>", Fuzzy.of("3.141592654")
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(Double.class).reply(core -> "Double <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElse("Anything else"));
    }

    @Test
    public final void testOrElse() {
        assertEquals("Anything else", Fuzzy.of(3.141592654)
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElse("Anything else"));
    }

    @Test
    public final void testOrElseGet() {
        assertEquals("Anything else", Fuzzy.of(3.141592654)
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orElseGet(() -> "Anything else"));
    }

    @Test(expected = MyException.class)
    public final void testOrThrow() {
        assertEquals("Double <3.141592654>", Fuzzy.of(3.141592654)
                .when(Integer.class).reply(core -> "Integer <" + core + ">")
                .orWhen(Float.class).reply(core -> "Float <" + core + ">")
                .orWhen(String.class).reply(core -> "String <" + core + ">")
                .orThrow(MyException::new));
    }

    private static class MyException extends RuntimeException {
    }
}